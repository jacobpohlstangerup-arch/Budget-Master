...
function updateTotals(){
  ...
  eurEl.style.display=currency==='DKK'?'block':'none';
  ...
  yrs().forEach(y=>{
    ...
    const inflStr=y>1&&inflationPct>0?`<div class="metric-infl">+${inflationPct}%/år</div>`:'';
    m.innerHTML=`<div class="metric-label">${T[lang].yr(y)}</div><div class="metric-value">${fmtCur(yt)}</div>${inflStr}`;
    sg.appendChild(m);
  });
...
window.exportExcel=function(){
  ...
  // Grand total
  const gtRow=[isDa?'SAMLET TOTAL (ALLE ÅR)':'GRAND TOTAL (ALL YEARS)','','',''];
  yr.forEach(y=>{gtRow.push(dispVal(yearGrand(y)));});
  gtRow.push(dispVal(gt));
  aoa.push(gtRow);rowIdx++;

  if(overheadPct>0){
    const ohGtRow=[`${isDa?'Inkl. overhead':'Incl. overhead'} (${overheadPct}%)`,'','',''];
